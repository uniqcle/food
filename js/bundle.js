...
function modals() {
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // означает что пользователь долистал до конца страницы.
      openModal();
...
function timer() {
  ...
  function getTimeRemaining(endTime) {
    ...
    if (t <= 0) {
      ...
      // Кол-во дней.  миллисекунд в сутках
      hours = Math.floor(t / (1000 * 60 * 60) % 24), mins = Math.floor(t / 1000 / 60 % 60), secs = Math.floor(t / 1000 % 60);
    }
    return {
      'total': t,
      'days': days,
